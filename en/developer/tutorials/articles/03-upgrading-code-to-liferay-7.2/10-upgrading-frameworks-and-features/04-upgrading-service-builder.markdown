...
1.  [Adapt the code to DXP's API](#step-1-adapt-the-code-to-product-vers-api)
...
## Step 1: Adapt the Code to DXP's API

Adapt the portlet to DXP's API using the Upgrade Planner. When
...
in the new DXP version's `liferay-ui` tag library. Notice the second attribute
`total`. 
...
attribute `package-path` specifies. In the new DXP version, Service Builder generates
